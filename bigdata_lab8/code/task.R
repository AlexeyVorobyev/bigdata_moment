...
data_set <- df1_initial[df1_initial$Country.Name == "United States",]
...
plot(years, VVP, xlab = 'Год', ylab = 'Прирост, %', main = 'Прирост ВВП в США', type = 'b', lty = 1, pch = 1, lwd = 2)
...
# по методу Спирмена
cor(VVP, Population_growth, method = "spearman")
cor.test(VVP, Population_growth)
# по методу Пирсона
cor(VVP, Population_growth, method = "pearson")

# корреляционная матрицы
df1 = data.frame(VVP, Population_growth)
